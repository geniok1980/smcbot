...
- **Стратегии на индикаторах** — RSI, MACD, Bollinger Bands, EMA, SMA и кастомные сигналы
...
### 📈 Аналитика (FastAPI)
- **Агрегированные метрики** — общая прибыль, баланс, открытые позиции, винрейт
- **Кластеризация и анализ** — выявление паттернов, точек входа/выхода
- **История позиций** — полный лог всех сделок с PnL
- **События стратегий** — запись сигналов, аналитических выводов и решений

### 🤖 ML-сервис
- **Обучение моделей** — LightGBM, XGBoost на исторических данных
- **Предсказание движения цены** — вероятностная оценка направления
- **Автоматическая перезагрузка** — модель переобучается по расписанию
- **API для робота** — робот запрашивает предикт перед входом в позицию
- **Feature engineering** — на основе свечных данных и индикаторов

### 📱 Telegram-бот
- **Управление ботом** — старт/стоп торговли, просмотр статуса
- **Уведомления** — открытие/закрытие позиций, экстренные события
- **Просмотр PnL** — текущая прибыль/убыток по открытым позициям
- **Баланс** — запрос текущего баланса на бирже

### 🖥️ Веб-дашборд
- **Обзор** — 4 KPI: общая прибыль, баланс, открытые позиции, винрейт
- **Управление** — кнопки старт/стоп бота и SELL ALL
- **Настройки** — просмотр переменных окружения (секреты скрыты)
- **Адаптивный дизайн** — Tailwind CSS, работает на мобильных
